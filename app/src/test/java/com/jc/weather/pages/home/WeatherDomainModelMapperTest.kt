...
class WeatherDomainModelMapperTest {
    ...
    fun `Check domain model mapping into UI model`() {
        ...
        val tomorrowDayTime = currentDayTime + WeatherDomainModelMapper.ONE_DAY_IN_SECONDS
        every { timestampProvider.toDayMonthAndDayName(tomorrowDayTime) } returns tomorrowResString

        every { context.getString(R.string.home_page_today) } returns todayResString
        every { context.getString(R.string.home_page_tomorrow) } returns tomorrowResString

        val actualUiModel = weatherDomainModelMapper.mapToUiModel(domainModel, city)
