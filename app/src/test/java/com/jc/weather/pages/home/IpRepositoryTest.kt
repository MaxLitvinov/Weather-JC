...
import com.jc.weather.pages.home.core.IpRepository
...
class IpRepositoryTest {
    ...
    fun `Check fetch location`() = runBlocking {
        ...
        val ipDtoModel = IpDto(query = query, status = status, country = country, countryCode = countryCode, region = region, regionName = regionName, city = city, zip = zip, lat = lat, lon = lon, timezone = timezone, isp = isp, org = org, _as = _as)

        coEvery { api.fetchLocation() } returns ipDtoModel

        val expectedResult = IpDomainModel(query = query, status = status, country = country, countryCode = countryCode, region = region, regionName = regionName, city = city, zip = zip, lat = lat, lon = lon, timezone = timezone, isp = isp, org = org, _as = _as)
